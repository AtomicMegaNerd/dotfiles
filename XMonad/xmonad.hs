...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch dmenu
    , ((modm,               xK_p     ), spawn "dmenu_run -p 'SuperJeb' -fn 'Noto Sans Mono:pixelsize=20' -nb '#224466' -sf '#224466' -sb '#d0d0d0' -nf '#d0d0d0'")
...
myLayout = avoidStruts $ tiled ||| grid ||| mirror ||| full 
  where
     ...
     mirror  = renamed [Replace "Vertical Tiled"] $ Mirror tiled
...
main = do
  ...
  xmonad $ docks defaults {
    logHook = dynamicLogWithPP $ xmobarPP {
        ...
        , ppVisible = xmobarColor "#999999" ""                -- Visible but not current workspace
        , ppHidden = xmobarColor "#77a88d" "" . wrap "*" ""   -- Hidden workspaces in xmobar
        , ppHiddenNoWindows = xmobarColor "#77a88d" ""        -- Hidden workspaces (no windows)
