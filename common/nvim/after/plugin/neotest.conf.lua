...
if not nt_status then
  vim.nofify("Could not load neotest", vim.log.levels.ERROR)
  return
end

local nt_pytest_status, nt_pytest = pcall(require, "neotest-python")
if not nt_pytest_status then
  vim.notify("Error: could not load neotest-python", vim.log.levels.ERROR)
  return
end

local nt_go_status, nt_go = pcall(require, "neotest-go")
if not nt_go_status then
  vim.notify("Error: could not load neotest-go", vim.log.levels.ERROR)
  return
end
